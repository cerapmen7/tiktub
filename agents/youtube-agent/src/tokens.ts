...
export interface YouTubeTokens {
  ...
  // googleapis peut retourner ces champs supplémentaires
  [key: string]: unknown;
}
 ...
 */
function findDataDir(start: string): string | null {
...
function getDefaultTokenPath(): string {
  // 1. Si env explicite
  if (process.env.YOUTUBE_TOKENS_PATH) return path.resolve(process.env.YOUTUBE_TOKENS_PATH);

  // 2. Cherche data/ existant
  const dataDir = findDataDir(process.cwd());
  if (dataDir) return path.join(dataDir, "tokens.json");
  ...
  try {
    ...
    if (fs.existsSync(fallbackDir) || fallbackDir.endsWith("data")) return fallback;
  } catch {
    // ignore
  }
...
export function getCachedTokens(): YouTubeTokens | null {
  return cachedTokens;
}

export function setCachedTokens(tokens: YouTubeTokens | null): void {
...
export async function loadTokens(customPath?: string): Promise<YouTubeTokens | null> {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as YouTubeTokens;
    // Validation minimale
 ...
 */
export async function saveTokensToPath(tokens: YouTubeTokens, customPath?: string): Promise<void> {
  const filePath = resolveTokenPath(customPath);
...
export async function persistTokens(
  pathOrTokens?: string | YouTubeTokens,
  tokensMaybe?: YouTubeTokens
): Promise<void> {
